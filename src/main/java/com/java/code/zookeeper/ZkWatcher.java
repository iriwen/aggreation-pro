package com.java.code.zookeeper;
...
public class ZkWatcher implements Watcher {
    ...
    public static void main(String[] args) throws Exception {
        ...
        zk = new ZooKeeper("localhost:2181", 3000, item->{
            System.out.println(" zk up ; the state is " + item.getState());
        });
    ...
    /**
     * 每当一个watcher处理完，被服务端移除之后，执行的对应的process方法会再重新注册新的watcher ，持续对node节点新发生的事件进行关注，
     * 注意设置好和zookeeper的会话失效时间（这里是关键，否则进行watcher注册的时候会提示connection losed，导致注册失败）,
     * 连接保持好 则持续注册都会成功
     * @param watchedEvent
     */
    @Override
